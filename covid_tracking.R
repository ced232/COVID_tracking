...
confirmed_data <- read.csv("confirmed_2021_2_18.csv", stringsAsFactors = FALSE)  %>%
...
deaths_data <- read.csv("deaths_2021_2_18.csv", stringsAsFactors = FALSE) %>%
    filter(!(Province_State %in% c("American Samoa", "Diamond Princess", "Grand Princess", "Guam", 
                                   "Northern Mariana Islands", "Puerto Rico", "Virgin Islands"))) %>%
...
pc_rotations <- as.data.frame(confirmed_prcomp$rotation[,1:8]) %>%
    mutate(date = as.Date("2020-1-23"):as.Date("2021-2-17")) %>%
